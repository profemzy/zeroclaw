...
Options:
  ...
  --onboard-interactive   Run 'onboard --interactive' inside prod container before startup
  --onboard-channels-only Run 'onboard --channels-only' inside prod container before startup
...
Examples:
  ...
  ./scripts/prod_local_up.sh --api-key sk-or-v1-... --onboard-interactive
EOF
}
...
if [ "$ONBOARD_INTERACTIVE" -eq 1 ] && [ "$ONBOARD_CHANNELS_ONLY" -eq 1 ]; then
  echo "error: --onboard-interactive and --onboard-channels-only are mutually exclusive." >&2
  exit 1
fi

if [ "$ONBOARD_INTERACTIVE" -eq 1 ] || [ "$ONBOARD_CHANNELS_ONLY" -eq 1 ]; then
  if [ ! -t 0 ] || [ ! -t 1 ]; then
    echo "error: onboarding mode requires an interactive terminal (TTY)." >&2
    exit 1
  fi
fi
...
if [ "$ONBOARD_INTERACTIVE" -eq 1 ]; then
  echo "Running onboarding wizard in production container ..."
  docker compose -f "$COMPOSE_FILE" run --rm -it "$SERVICE_NAME" onboard --interactive
elif [ "$ONBOARD_CHANNELS_ONLY" -eq 1 ]; then
  echo "Running channels-only onboarding in production container ..."
  docker compose -f "$COMPOSE_FILE" run --rm -it "$SERVICE_NAME" onboard --channels-only
fi
